...
$\lambda$ (W m$^{-1}$ K$^{-1}$). The volumetric heat capacity is defined as $C = \rho c_p$. In 
very simple, homogeneous cases, the diffusivity $\mu$ (m$^2$/s) can be defined as  
...
To keep things wimple, we have chosen a very simple homogeneous subsurface model, with an idealized 
GSTH taken from \cite{Balling1981a}. In order to avoid an ``inverse crime'' \cite[see] [] 
{Kaipio2005a}, these models were simulated with a numerical code developed independently 
...
For each of the numerical experiments presented in Section~\ref{sec:results}, we ran small 
ensembles (24-32 samples) for different error realizations. While this is surely not enough to 
produce statistically reliable results, it gives an indication of the importance of the errors in 
each case.   
